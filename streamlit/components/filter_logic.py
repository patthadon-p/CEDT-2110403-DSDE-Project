...
class TraffyFilter:
    ...
    def apply_filters(
        self, filter_type: bool = True, filter_date: bool = True
    ) -> pd.DataFrame:
        ...
        if filter_date:
            ...
            # Apply date mask
            date_mask = (df_filtered["date"] >= start_ts) & (
                df_filtered["date"] <= end_ts
            )
            df_filtered = df_filtered[date_mask]

        if filter_type and self.current_type != "ทั้งหมด":
            # Apply type mask (using the list of types)
            type_mask = df_filtered["type_cleaned"].apply(
                lambda x: self.current_type in x
            )
