...
from src.utils import read_config_path, _HAS_SKLEARN
from src.data_loader import load_geo_data # Needed for map page and predictor page


# If DBSCAN is not available, we use the mock defined in utils
if not _HAS_SKLEARN:
    from src.utils import plot_dbscan_map # Import the mock function
else:
...
class LineChartVisualizer:
    def __init__(self, df):
        ...
        if "timestamp_year" not in self.df.columns or "timestamp_month" not in self.df.columns:
             # Fallback check for new column names if the main app logic failed to rename back
             if "year" in self.df.columns:
                 self.df.rename(columns={"year": "timestamp_year", "month": "timestamp_month"}, inplace=True)
             else:
                 raise ValueError("DataFrame must have 'timestamp_year' and 'timestamp_month' columns for monthly grouping.")
        
        # 2. Create date column for sorting and grouping
        # NOTE: Using the original names expected by the class logic
        self.df['date_ts'] = pd.to_datetime(self.df[['timestamp_year', 'timestamp_month']].rename(columns={"timestamp_year": "year", "timestamp_month": "month"}).assign(day=1))
